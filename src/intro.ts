...
// Its path length is normalised to 1000 by the pathLength attribute on the rect, so these are the
// only numbers involved: no measuring, and a late font swap cannot stretch it.
const LEN = 1000;
const FRAME_AT = 0.37;
const DRAW = 0.5;
...
export function playOpening(hero: gsap.core.Timeline, fontsReady: Promise<unknown>, lenis: Lenis | null): void {
  ...
  tl.set([greet, name], { visibility: "visible" }, 0)
    ...
    // power2.inOut is its own reverse, so the pen leaves as gently as it arrived. power2.in was
    // fastest at the end: its last frame took off 221 px against the draw's 123 px peak.
    .to(shape, { strokeDashoffset: LEN, duration: UNDRAW, ease: "power2.inOut" }, "retrace")
